pipeline {
  ...
  stages {
    ...
    stage('cfn-lint') {
      steps {
        script {
          sh(script: "testenv/bin/cfn-lint aws/Full-PAS-Deployment.yaml --ignore-checks W2506", returnStdout: true)
          ...
          sh(script: "testenv/bin/cfn-lint aws/PTA-Single-Deployment.yaml --ignore-checks W2506", returnStdout: true)
