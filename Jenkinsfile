    ...
    stage('Syntax Validation') {
      steps {
        script {
          ...
          sh(script: "aws cloudformation validate-template --region $AWS_REGION --template-url $TEMPLATE_URL/PAS-AIO-template.yaml", returnStdout: true)
          ...
          sh(script: "aws cloudformation validate-template --region $AWS_REGION --template-url $TEMPLATE_URL/PAS-network-environment-NAT.yaml", returnStdout: true)
          sh(script: "aws cloudformation validate-template --region $AWS_REGION --template-url $TEMPLATE_URL/PAS-network-environment-PrivateLink.yaml", returnStdout: true)
    ...
    stage('cfn-lint') {
      steps {
        script {
          ...
          sh(script: "testenv/bin/cfn-lint aws/PAS-AIO-template.yaml --ignore-checks W1001", returnStdout: true)
          sh(script: "testenv/bin/cfn-lint aws/PAS-AIO-dr-Deployment.yaml --ignore-checks W1001", returnStdout: true)
          sh(script: "testenv/bin/cfn-lint aws/PAS-network-environment-NAT.yaml --ignore-checks W1001", returnStdout: true)
          sh(script: "testenv/bin/cfn-lint aws/PAS-network-environment-PrivateLink.yaml --ignore-checks W1001", returnStdout: true)
        }
      }
    }
    stage('pytest') {
